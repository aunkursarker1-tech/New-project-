...
export const HeroBanner: React.FC<HeroBannerProps> = ({
  ...
  const activeSliders = banners ? banners.filter(b => b.active && b.type === 'slider') : [];
  const activePromos = banners ? banners.filter(b => b.active && b.type === 'promo') : [];

  const sortedSliders = [...activeSliders].sort((a, b) => a.order - b.order);
  const sortedPromos = [...activePromos].sort((a, b) => a.order - b.order);
  ...
  return (
    <section className="space-y-3 pt-2 pb-2 px-3 sm:px-4 max-w-7xl mx-auto">
      ...
      {sortedSliders.length > 0 ? (
        <a 
          href={sortedSliders[0].linkUrl || '#'}
          className="relative rounded-xl overflow-hidden shadow-sm border border-slate-200/60 dark:border-slate-800 bg-gradient-to-br from-[#0a0f26] to-[#12193b] text-white p-5 flex flex-col sm:flex-row items-center justify-between gap-4 min-h-[140px] hover:shadow-md transition-shadow"
        >
          <div className="space-y-1.5 text-left z-10 flex-1">
            <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-extrabold text-[8px] sm:text-[9px] uppercase tracking-wider">
              ...
              <span>OFFICIAL PROMOTION</span>
            </div>
            <h2 className="text-sm sm:text-xl font-black text-white leading-tight">
              {sortedSliders[0].title}
            </h2>
            <p className="text-[10px] sm:text-xs text-slate-300 max-w-lg leading-relaxed">
              {sortedSliders[0].subtitle}
            </p>
            {sortedSliders[0].buttonText && (
              <span className="inline-block bg-[#007A58] hover:bg-[#005a3e] text-white font-black text-[9px] sm:text-[10px] px-3 py-1 rounded-md mt-1.5 tracking-wider uppercase shadow">
                {sortedSliders[0].buttonText}
              </span>
            )}
          </div>

          <div className="w-20 h-20 sm:w-28 sm:h-28 shrink-0 relative flex items-center justify-center p-1 bg-white/5 rounded-lg overflow-hidden">
            <img 
              src={sortedSliders[0].imageUrl} 
              alt={sortedSliders[0].title} 
      ...
      <div className="grid grid-cols-2 gap-2">
        {/* Left Banner: SAMSUNG TV FEST / Dynamic Left Promo */}
        {sortedPromos[0] ? (
          <a 
            href={sortedPromos[0].linkUrl || '#'}
          ...
          >
            <div className="space-y-1.5 text-left z-10 flex-1">
              <div className="inline-block text-[7px] sm:text-[9px] font-extrabold text-white bg-blue-600 px-1.5 py-0.5 rounded tracking-wider uppercase">
                {sortedPromos[0].title}
              </div>
              <h3 className="text-[10px] sm:text-xs font-black text-white leading-tight">
                {sortedPromos[0].subtitle}
              </h3>
              {sortedPromos[0].buttonText && (
                <div className="inline-block bg-slate-800 text-white font-bold text-[8px] sm:text-[10px] px-1.5 py-0.5 rounded">
                  {sortedPromos[0].buttonText}
                </div>
              )}
            </div>

            <div className="w-16 h-16 sm:w-20 sm:h-20 shrink-0 relative flex items-center justify-center p-0.5 bg-black/10 rounded overflow-hidden">
              <img 
                src={sortedPromos[0].imageUrl} 
                alt={sortedPromos[0].title} 
        ...
        {/* Right Banner: HITACHI Washing Machine / Dynamic Right Promo */}
        {sortedPromos[1] ? (
          <a 
            href={sortedPromos[1].linkUrl || '#'}
          ...
          >
            ...
            <div className="w-16 h-16 sm:w-20 sm:h-20 shrink-0 relative flex items-center justify-center p-0.5 bg-black/5 rounded overflow-hidden">
              <img 
                src={sortedPromos[1].imageUrl} 
                alt={sortedPromos[1].title} 
                className="max-h-full max-w-full object-contain group-hover:scale-105 transition-transform duration-300"
              />
            </div>
          </a>
      ...
      {sortedPromos[2] ? (
        <a 
          href={sortedPromos[2].linkUrl || '#'}
          className="relative overflow-hidden rounded-lg border border-[#005a50] bg-[#007467] p-2 flex items-center justify-between text-white hover:shadow-md transition-shadow"
        >
          ...
          <div className="bg-[#0f1d4a] px-3 py-1 rounded text-center flex items-center gap-1.5 sm:gap-3">
            <span className="text-[7.5px] sm:text-[10px] font-black text-slate-300">
              {sortedPromos[2].subtitle}
            </span>
          </div>

          <div className="flex items-center gap-1.5">
            {sortedPromos[2].buttonText && (
              <span className="px-2.5 py-1 text-[8px] sm:text-[10px] font-black bg-white text-slate-900 rounded shadow-sm hover:bg-slate-100 transition-all active:scale-95">
                {sortedPromos[2].buttonText}
              </span>
            )}
          </div>
