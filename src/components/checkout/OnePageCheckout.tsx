...
export const OnePageCheckout: React.FC<OnePageCheckoutProps> = ({
  ...
  return (
    <div className={`w-full max-w-5xl mx-auto rounded-3xl transition-all ${
          ...
          <div className={`p-5 rounded-3xl border transition-all ${
            ...
            {gpsSuccess && (
              <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs space-y-2">
                ...
                {isMapPreviewOpen && (
                  <div className={`mt-2 p-3 rounded-2xl border space-y-2 ${
                    darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'
                  }`}>
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-slate-500">GPS Pin Coordinates:</span>
                    ...
                    <div className={`relative h-28 w-full rounded-xl overflow-hidden border flex items-center justify-center ${
                      ...
                      <div className="relative z-10 flex flex-col items-center justify-center text-center p-2">
                        <div className="p-2 rounded-full bg-emerald-500 text-white shadow-lg animate-bounce">
                          <MapPin className="w-5 h-5" />
                        </div>
                        <span className={`text-[10px] font-black mt-1 ${darkMode ? 'text-slate-200' : 'text-slate-900'}`}>
                          {selectedUpazila}, {selectedDistrict}
                        </span>
                        <span className={`text-[9px] truncate max-w-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>{fullAddress}</span>
                      </div>
                      <span className="absolute bottom-1.5 right-2 text-[9px] font-bold text-slate-400 uppercase">
        ...
        <div className="lg:col-span-5 space-y-5">
          <div className={`p-5 rounded-3xl border sticky top-4 ${
            ...
            <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
              {items.map((item) => (
                <div key={item.product.id} className={`flex items-center gap-3 p-2.5 rounded-2xl border ${
                  darkMode ? 'bg-slate-950/40 border-slate-800/60' : 'bg-slate-50 border-slate-200/80'
                }`}>
                  {/* Large Product Image */}
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className={`font-extrabold text-xs truncate ${darkMode ? 'text-slate-100' : 'text-slate-900'}`}>{item.product.name}</h4>
                    {(item.selectedColor || (item as any).variant) && (
                      <span className={`inline-block text-[10px] px-2 py-0.5 rounded-md mt-0.5 font-medium ${
                        darkMode ? 'bg-slate-800 text-slate-300' : 'bg-slate-200 text-slate-700'
                      }`}>
                        {item.selectedColor || (item as any).variant}
                      </span>
                    )}
                    <p className={`text-xs font-black mt-1 ${darkMode ? 'text-emerald-400' : 'text-[#007A58]'}`}>
                  ...
                  <div className={`flex items-center gap-1 border rounded-xl p-1 shrink-0 ${
                    ...
                    <span className={`w-6 text-center text-xs font-black ${darkMode ? 'text-white' : 'text-slate-900'}`}>{item.quantity}</span>
                    <button
                      type="button"
                      onClick={() => handleItemQtyChange(item.product.id, 1)}
                      className={`w-6 h-6 rounded-lg font-bold flex items-center justify-center text-xs ${
                        darkMode ? 'bg-slate-800 hover:bg-slate-700 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-855'
                      }`}
                    >
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className={`w-full pl-9 pr-3 py-2 rounded-xl text-xs uppercase font-bold outline-none border ${
                      darkMode ? 'bg-slate-950 border-slate-800 text-white focus:border-amber-400' : 'bg-white border-slate-200 text-slate-900 focus:border-[#007A58]'
                    }`}
                  />
                </div>
                <button
      ...
      <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-40 p-3 backdrop-blur-xl border-t shadow-2xl flex items-center justify-between gap-3 ${
        ...
        <div>
          ...
          <span className={`text-lg font-black ${darkMode ? 'text-emerald-400' : 'text-[#007A58]'}`}>{formatPrice(grandTotal)}</span>
        </div>
